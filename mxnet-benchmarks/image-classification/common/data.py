...
class SyntheticDataIter(DataIter):
    def __init__(self, num_classes, data_shape, max_iter, dtype, kv):
        ...
        self.data = mx.nd.array(data, dtype=self.dtype, ctx=mx.Context('cpu_pinned', kv.rank % 8))
        self.label = mx.nd.array(label, dtype=self.dtype, ctx=mx.Context('cpu_pinned', kv.rank % 8))
    def __iter__(self):
        return self
    @property
