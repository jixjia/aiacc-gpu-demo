...
def main(_):
  ...
  if True:
    classes_tensor = sess.graph.get_tensor_by_name('ArgMax:0')
    logits_tensor  = sess.graph.get_tensor_by_name('softmax_tensor:0')

    for _ in range(5):
      result = sess.run([classes_tensor, logits_tensor], {'input_tensor:0': [image_data]})
